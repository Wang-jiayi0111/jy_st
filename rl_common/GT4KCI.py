...
# 新数据集名称
NEW_DATASET = "combine_all"
# 采样比例（每个数据集取20%）
SAMPLE_RATIO = 1.0
# 要组合的数据集列表
DATASETS = ["JHotDraw"]
#DATASETS = ["ANT","ATM", "BCEL","daisy","DNS", "email__spl", "notepad__spl", "SPM"]
#DATASETS = ["ANT", "jdk", "JHotDraw", "junit4", "Wro4J"]
...
# -----------------------------------
# 确保结果目录存在
result_dir = "/home/ps/jy_exp/output/GNN_res6"
os.makedirs(result_dir, exist_ok=True)

# 历史模型保存路径
historical_models_dir = "/home/ps/jy_exp/output/GNN_res6/historicalModels"
    ...
    if prev_model_path and os.path.exists(prev_model_path):
        try:
            ...
            if 'gamma' in checkpoint and 'beta' in checkpoint and not ramdom_gama_beta:
                ...
                with torch.no_grad():
                    # 使用赋值而非copy_()
                    model.gamma = torch.nn.Parameter(
                        torch.tensor(checkpoint['gamma']), 
                        requires_grad=True
                    )
                    model.beta = torch.nn.Parameter(
                        torch.tensor(checkpoint['beta']), 
                        requires_grad=True
                    )
                ...
                if hasattr(model, 'centrality_scaling') and 'centrality_scaling' in checkpoint:
                    model.centrality_scaling = torch.nn.Parameter(
                        torch.tensor(checkpoint['centrality_scaling']),
                        requires_grad=True
                    )
                ...
                # 反算raw值 - 使用非原地操作
                    ...
                    raw_value = torch.log(
                        torch.tensor(mixing_weight) / 
                        (1 - torch.tensor(mixing_weight) + 1e-8)
                    )
                    model.mixing_weight_raw = torch.nn.Parameter(
                        raw_value, 
                        requires_grad=True
                    )
                ...
                if hasattr(model, 'noise_threshold') and 'noise_threshold' in checkpoint:
                    ...
                    model.noise_threshold = torch.nn.Parameter(
                        raw_value, 
                        requires_grad=True
                    )
...
def identify_key_classes(G, threshold_count=4, top_percent=0.15, node_mapping=None):
    """
    使用8种方法识别关键类
    :param G: NetworkX 图
    ...
    :return: 关键类列表，候选类字典，投票统计，标签数组，节点映射
    """
    # 如果没有提供节点映射，创建默认映射
    if node_mapping is None:
        node_mapping = {node: f"Node-{node}" for node in G.nodes()}
        logging.warning("未提供节点映射，使用默认节点名称")
    ...
    for node in key_classes:
        ...
        if node < len(labels):
            ...
            logging.warning(f"节点 {node} 超出标签数组范围")
    
    # 6. 记录统计信息
    num_nodes = len(G.nodes())
    num_key_classes = len(key_classes)
    logging.info(f"识别出关键类: {num_key_classes}/{num_nodes} ({num_key_classes/num_nodes:.2%})")
    
    # 记录关键类名称
    key_class_names = [node_mapping.get(node, f"Node-{node}") for node in key_classes]
    logging.info(f"关键类: {', '.join(key_class_names[:10])}{'...' if len(key_class_names) > 10 else ''}")
    
    return key_classes, candidate_key_classes, candidate_counter, labels, node_mapping

def print_key_class_results(key_classes, node_mapping, dataset_name=None):
    """
    打印关键类识别结果，使用logging.info记录关键类ID和类名
    :param key_classes: 关键类节点ID列表
    :param node_mapping: 节点ID到类名的映射字典
    :param dataset_name: 数据集名称（可选）
    """
    if dataset_name:
        logging.info(f"\n{'*' * 80}")
    ...
    logging.info("-" * 60)
    for node in key_classes:
        class_name = node_mapping.get(node, f"Node-{node}")
        logging.info(f"节点 {node}: {class_name}")
    
    # 统计信息
    num_key_classes = len(key_classes)
    logging.info("\n统计摘要:")
    logging.info("-" * 60)
    logging.info(f"关键类数量: {num_key_classes}")
...
def process_datasets():
    """
    处理所有数据集，收集关键类识别结果并合并
    :return: 合并后的标签数组和节点映射
    """
    logging.info("开始处理所有数据集...")
    # 存储所有数据集的结果
    all_labels = []
    all_node_mapping = {}
    all_key_classes = []
    ...
    for dataset in DATASETS:
        try:
            ...
            # 1. 加载图数据 (这里需要实现实际的数据加载逻辑)
            dataset_path = f"/home/ps/jy_exp/input_GNN/{dataset}_MCN/combined_{dataset}_GN.net"
            G, node_mapping = read_net_file(dataset_path)
            
            # 3. 识别关键类
            results = identify_key_classes(
                G, 
                threshold_count=4, 
                top_percent=0.15,
                node_mapping=node_mapping
            )
            key_classes, candidate_key_classes, candidate_counter, labels, node_mapping = results
            
            # 4. 打印当前数据集的结果
            print_key_class_results(key_classes, node_mapping, dataset_name=dataset)
            
            # 5. 为全局映射创建前缀
            dataset_prefix = f"{dataset}_"
            
            # 6. 更新全局节点映射
            for local_node_id, class_name in node_mapping.items():
                global_node_id_str = f"{dataset_prefix}{local_node_id}"
                all_node_mapping[global_node_id_str] = class_name
                dataset_node_mapping[local_node_id] = global_node_id_str
            
            # 7. 更新标签数组
            # 注意: 这里假设每个数据集的标签是独立的
            # 我们将标签数组扩展为全局索引
            for local_node_id, label in enumerate(labels):
                global_node_id_str = dataset_node_mapping.get(local_node_id, f"{dataset_prefix}{local_node_id}")
                # 在实际实现中，您可能需要将标签存储在适当的数据结构中
                # 这里我们只是收集所有标签
                all_labels.append(label)
                
                # 如果是关键类，添加到全局关键类列表
                if label == 1:
                    all_key_classes.append(global_node_id_str)
            
            # 8. 记录处理完成
            logging.info(f"数据集 {dataset} 处理完成")
            
        except Exception as e:
            logging.error(f"处理数据集 {dataset} 时出错: {str(e)}")
            continue
    
    # 9. 打印合并后的结果
    logging.info(f"\n{'#' * 80}")
    logging.info("所有数据集合并结果")
    logging.info(f"{'#' * 80}")
    print_key_class_results(all_key_classes, all_node_mapping)
    logging.info("处理完成!")
    return all_labels, all_node_mapping, all_key_classes
...
def main():
    ...
    all_labels, all_mappings, all_key_classes = process_datasets()
    logging.info(all_labels)
    # # 识别关键类
    # key_nodes, candidate_key_classes, candidate_counter, labels, node_mappings = identify_key_classes()
    
    # # 打印结果
    # print_key_class_results(key_nodes, candidate_key_classes, candidate_counter, node_mappings)

    logging.info(f"关键类节点: {all_key_classes}")
    ...
    if searching_best_param:
        best_params = parameter_tuning(G, all_labels, combined_features)  # 参数调优
        logging.info("\n使用最佳参数进行训练和评估...")
    else:
        # 使用预设的参数
    ...
    if num_splits > 1:
        ...
        for split_idx, (train_indices, test_indices) in enumerate(kf.split(indices)):
            ...
            # 训练集和测试集标签初始化
            train_labels = np.zeros_like(all_labels)
            test_labels = np.zeros_like(all_labels)
            train_labels[train_indices] = all_labels[train_indices]
            test_labels[test_indices] = all_labels[test_indices]
            
            # 从训练集中划分验证集（20%）
            val_indices = train_indices[:len(train_indices)//5]
            actual_train_indices = train_indices[len(train_indices)//5:]
            val_labels = np.zeros_like(all_labels)
            val_labels[val_indices] = all_labels[val_indices]
            
            # 调整训练集，排除验证集
            actual_train_labels = np.zeros_like(all_labels)
            actual_train_labels[actual_train_indices] = all_labels[actual_train_indices]
            ...
            if split_idx == 0:
                metadata = {
                    ...
                    'num_key_nodes': len(all_key_classes)
                }
    ...
    for rank, idx in enumerate(sorted_indices[:15]):  # 显示前15个
        ...
        is_key = "✓" if all_labels[idx] == 1 else " "
    ...
    for k in k_list:
        top_k_indices = sorted_indices[:k]
        correct_keys = sum(1 for idx in top_k_indices if all_labels[idx] == 1)
        total_keys = sum(all_labels)
